...
st.markdown(
    ...
    .caption {
        ...
        color: #555555;
        font-style: italic;
        margin-bottom: 25px; /* Space below caption before image */
    }
    ...
    /* Enhanced horizontal line styling for clearer separation */
    .sidebar-hr {
        border: 0;
        height: 2px; /* Thicker line */
        background: #0d47a1;
        margin: 10px 0; /* Reduced margin to bring File Upload higher */
        width: 90%; /* Slightly wider */
        margin-left: auto;
        margin-right: auto;
    }
    /* File upload section with darker background */
    .file-upload-section {
        background-color: #eceff1; /* Slightly darker than body background #f5f7fa */
        padding: 5px 15px; /* Reduced top padding to bring content higher */
...
# Function to export chat logs as PDF
...
# Function to preprocess and sample document content
...
# Function to filter responses (more flexible)
...
# Function to suggest questions based on entire document content
...
# Function to process a question (unified for manual input and suggestions)
...
# Sidebar setup for settings and file upload
...
st.sidebar.markdown('</div>', unsafe_allow_html=True)  # Close File Upload section
...
# Relaxed prompt template to allow contextual reasoning
prompt_template = """
...
# Chatbot interface with question suggestions and export chat log
if st.session_state.vector_db is not None:
